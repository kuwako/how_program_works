...
- ネイティブコードにadd, cmpなどの分かりやすい略語(ニーモニック)をつけた
- ニーモニックを使う言語をアセンブリ言語と呼ぶ
- アセンブリ言語をネイティブ・コードに変換するプログラムをアセンブラと呼ぶ
- ネイティブ・コードからアセンブリ言語に逆変換することも可能(逆アセンブラ)
- ネイティブ・コードからC言語に逆アセンブラするのは困難

### コンパイラでアセンブリ言語のソースコードを出力
- Borland C++ではコンパイラのオプションに-Sをつけることでアセンブリ言語のソースコードが生成される
- アセンブリ言語の拡張子は.asm

### ネイティブ・コードに変換されない「擬似命令」
- アセンブリ言語のソースコードは、ネイティブ・コードに変換されるオペコードと、アセンブラ自体に対する命令の擬似命令から構成されている
- 擬似命令segmentとendsで囲まれた部分は、プログラムを構成する命令やデータをまとめたグループに名前をつけたもの
    - \_TEXTは命令セグメント
    - \_DATAは初期化されたデータのセグメント
    - \_BSSは初期化されていないデータを持つセグメント

### アセンブリ言語の構文は「オペコード+オペランド」
- オペコードは命令の動作を表、オペランドは命令の対象となるものを表す
- オペコードが動詞、オペランドが目的語
...
- ローカル変数のための領域は一時的に確保されている
    - レジスタが空いていればレジスタが利用される
    - コンパイラが最適化してくれる
    - レジスタはメモリより大幅にアクセスが早い

### 繰り返し処理の実現方法
- アセンブリ言語では繰り返しを比較命令(cmp)とジャンプ命令（jl）で実現している
- mov命令よりxor命令の方が処理が高速
- jl命令は(jump on less than)の命令

### 条件分岐の実現方法
- jle(jump on less or equal)命令、jge(jump on greater or equal)、結果によらずjmp(jump)命令等を駆使する
- jump命令の前にはcmp命令があり、その結果はフラグ・レジスタに格納されている

### プログラムの生の動きを知らなければわからないことがある
- グローバル変数 int counter = 100 を2倍にするfuncAとfuncBをマルチスレッドで実行した場合、運が悪いと結果が400にならずに200になりうる
- counter = counter * 2; という1行はアセンブリ言語では以下の3つの命令から構成される
    - counterの値をeaxレジスタに読み出す
    - eaxレジスタの値を2倍する
    - eaxレジスタの値をcounterに書き込む
- マルチスレッド処理では、1行の処理が実行されるごとに、他のスレッドに処理が切り替わる可能性がある
- タイミング次第では、funcAからcounterの値をeaxレジスタに読み出した瞬間にスレッドが切り替わり、funcBが実行された場合などは起こりうる
- ロックを使えば回避できる

## 11章ハードウェアを制御する方法
### ハードウェアの入出力を支えるのはIN命令とOUT命令
- IOコントローラ: コンピュータ本体と周辺装置の電気的特性を相互に変換するIC
    - 入出力されるデータをためておくメモリがあり、これをポートと呼んでいる
- C言語内では \_asm{}で囲った中はニーモニックが使用可能
...
### 文字やグラフィックが表示される仕組み
- VRAMと呼ばれるディスプレイに表示される情報を常に記憶しているメモリが存在する
    - VRAMにデータを書き込めば瞬時にディスプレイに表示される
    - GPU
